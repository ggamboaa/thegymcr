...
exports.getAll = (req, res) => {
  ...
  if(filter){
    ...
    .then(departments => {
      res.status(200).json({'records': departments.rows, 'totalRecords':departments.count, 'numberOfPageRecords': limit});
    })
    .catch(error => {
    ...
    .then(departments => {
      ...
      res.status(200).json({'records': departments.rows, 'totalRecords':departments.count, 'totalPages': pages,
...
exports.changeStatus = (req, res) => {
  ...
  models.Department.update( { status: req.body.status }, { where: {id: id} })
  .then(() => {
